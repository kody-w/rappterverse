...
const WorldMode = {
    ...
    init(worldId) {
        ...
        const canonicalConfig = GameState.getWorldConfig(worldId);
        this.combatEnabled = canonicalConfig?.features?.battles === true;
        ...
        // Show combat HUD
        this._setCombatUi(this.combatEnabled);
        ...
        // Init RPG systems
        if (this.combatEnabled && typeof PlayerStats !== 'undefined') { PlayerStats.init(); PlayerStats.load(); }
        if (this.combatEnabled && typeof Abilities !== 'undefined') Abilities.init();
        if (this.combatEnabled && typeof ComboSystem !== 'undefined') ComboSystem.reset();
        if (this.combatEnabled && typeof Inventory !== 'undefined') Inventory.init();
        if (this.combatEnabled && typeof Equipment !== 'undefined') Equipment.init();
        if (typeof StatusEffects !== 'undefined') StatusEffects.cleanup();
        if (this.combatEnabled && typeof EnemyHero !== 'undefined') EnemyHero.init(this.scene, w.bounds);
        ...
        if (typeof HUD !== 'undefined') {
            ...
            HUD.showToast(
                this.combatEnabled
                    ? `Landed on ${w.name} — SPACE to attack, WASD to move`
                    : `Landed on ${w.name} — WASD to explore`
            );
        ...
        // Init Replay System
        if (this.combatEnabled && typeof ReplaySystem !== 'undefined') ReplaySystem.init();
    ...
    update(delta, time) {
        ...
        // Player attack (SPACE)
        if (this.combatEnabled && this.keys['Space']) {
        ...
        if (this.combatEnabled && typeof ReplaySystem !== 'undefined') ReplaySystem.update(delta, time);
        ...
        if (this.combatEnabled && typeof JungleCamps !== "undefined") JungleCamps.update(delta, this.player.mesh.position);
        ...
        if (this.combatEnabled && typeof FogOfWar !== 'undefined') FogOfWar.update(this.player.mesh.position);
        ...
        // RPG system updates
        if (this.combatEnabled && typeof PlayerStats !== 'undefined') {
            PlayerStats.update(delta);
            if (PlayerStats.dead) return; // Skip everything while dead
        }
        if (this.combatEnabled && typeof Abilities !== 'undefined') Abilities.update(delta);
        if (this.combatEnabled && typeof ComboSystem !== 'undefined') ComboSystem.update(delta);
        if (this.combatEnabled && typeof Inventory !== 'undefined') {
            Inventory.collectNearby(this.player.mesh.position);
            Inventory.updateDrops(time);
        }

        // Battle-world simulation only; non-battle worlds remain safe social spaces.
        if (this.combatEnabled && !WorldCombat._warmupActive) {
            // Enemy hero update
    ...
    _setCombatUi(enabled) {
        const combatHud = document.getElementById('combat-hud');
        if (combatHud) combatHud.style.display = enabled ? 'flex' : 'none';
        const statsBar = document.getElementById('player-stats-bar');
        if (statsBar) statsBar.style.display = enabled ? 'flex' : 'none';
        const goldBar = document.getElementById('gold-kda-bar');
        if (goldBar) goldBar.style.display = enabled ? 'flex' : 'none';
        const levelBadge = document.getElementById('level-badge');
        if (levelBadge) levelBadge.style.display = enabled ? 'block' : 'none';
        const abilityBar = document.getElementById('ability-bar');
        if (abilityBar) abilityBar.style.display = enabled ? 'flex' : 'none';
    },

    _startCombatSystems() {
        const world = WORLDS[this.currentWorld];
        WorldLanes.init(this.scene, world);
        WorldCombat.init(this.scene);
        if (typeof JungleCamps !== 'undefined') JungleCamps.init(this.scene, world);
        if (typeof FogOfWar !== 'undefined') FogOfWar.init(this.scene, world);
        ...
        if (typeof StatusEffects !== 'undefined') StatusEffects.cleanup();
        if (typeof EnemyHero !== 'undefined') EnemyHero.init(this.scene, world.bounds);
        if (typeof ReplaySystem !== 'undefined') ReplaySystem.init();
        this._setCombatUi(true);
    },
    ...
    _stopCombatSystems() {
        ...
        if (warmupTimer) warmupTimer.style.display = 'none';
        this._setCombatUi(false);
    },

    onCanonicalConfigChanged() {
        if (!this.active) return;
        const enabled = GameState.getWorldConfig(this.currentWorld)?.features?.battles === true;
        if (enabled === this.combatEnabled) return;
        this.combatEnabled = enabled;
        if (enabled) {
            this._startCombatSystems();
            if (typeof HUD !== 'undefined') HUD.showToast('Arena combat enabled by canonical world state');
        } else {
            this._stopCombatSystems();
            if (typeof HUD !== 'undefined') HUD.showToast('Combat disabled by canonical world state');
        }
    },
    ...
    cleanup() {
        const hadCombat = this.combatEnabled;
        this.active = false;
        ...
        if (hadCombat) this._stopCombatSystems();
        else WorldCombat.cleanup();
