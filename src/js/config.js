...
function seededRandom(seed) {
    // inventory.js's spawnDrop() calls this with `worldId * 10000 + waveNumber
    // * 100 + creepIndex` where worldId is a world id STRING (GameState.
    // currentWorld, e.g. "arena") -- `string * number` is NaN, and
    // `NaN.length` is undefined, so the hash loop below never ran and every
    // single call started from the same s=0 state. That made rng()'s first
    // value always ~0.0000057 -- always under the "30% chance no drop"
    // threshold -- so no creep or jungle camp ever dropped an item, in any
    // world, on any wave, since this function was written. createNoise2D()
    // already defends against this itself (`seededRandom(String(seed))`);
    // normalizing here instead protects every current and future caller,
    // numeric or not, the same way.
    seed = String(seed);
    let s = 0;
