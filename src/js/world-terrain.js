...
const WorldTerrain = {
    ...
    // ──────────────────────────────────────────────────────
    // `w.bounds.x`/`w.bounds.z` is the canonical playable half-extent used
    // everywhere else in the game (the player's own position is hard-clamped
    // to exactly this range in world-core.js, and the fog-of-war minimap maps
    // its view to it with only a +5-unit margin). Placement here uses
    // `(rng()-0.5) * bounds * M`, i.e. a half-range of `0.5*M*bounds` -- so
    // M=2 reaches exactly the true edge, and the M=1.2/1.4 used below reach
    // only 60%/70% of the way there. Worked out precisely per biome/world
    // below: every STATIC circular/box placement (Crystal, Abyss, Desert,
    // Terra) turns out to already have a 30-50+ unit safety margin even at
    // each feature's own worst-case radius, so it was never actually
    // capable of exceeding bounds. The one placement that genuinely can is
    // the Volcanic lava river: its center starts safely inset, but then
    // random-walks via noise for 15 steps at up to ~6 units/step -- up to
    // ~90 units of *unbounded* drift against only a ~35-unit starting
    // margin, which easily carries it past the true edge. clampToBounds
    // re-pins a point (accounting for the feature's own radius/half-extent)
    // to stay inside the real playable rectangle; used only where the math
    // shows it's actually needed (the river's per-step drift), not applied
    // reflexively to every placement.
    clampToBounds(x, z, w, extent) {
        const mx = Math.max(0, w.bounds.x - extent);
        const mz = Math.max(0, w.bounds.z - extent);
        return {
    ...
    spawnBiomeFeatures(scene, w, worldId) {
        ...
        if (w.biome === 'Volcanic') {
            ...
            for (let i = 0; i < 6; i++) {
                const tubeR = 0.4 + rng() * 0.8;
                const pts = [];
                let start = this.clampToBounds((rng() - 0.5) * w.bounds.x, (rng() - 0.5) * w.bounds.z, w, tubeR);
                let px = start.x, pz = start.z;
                for (let j = 0; j < 15; j++) {
                    const clamped = this.clampToBounds(px, pz, w, tubeR);
                ...
                if (pts.length > 3) {
                    ...
                    const tube = new THREE.Mesh(
                        new THREE.TubeGeometry(curve, 24, tubeR, 6, false),
        ...
        if (w.biome === 'Crystal') {
            // Frozen lake patches. Worst case (bounds 120x150, r up to 9):
            // half-range (72,90) + r(9) = (81,99) vs bounds (120,150) --
            // margins of 39 (x) and 51 (z). Never actually capable of
            // exceeding bounds despite the *1.2 looking similar to the lava
            // river's *1.4 (which genuinely can, via unbounded noise drift
            // -- see the Volcanic clamp above). No clamp needed here.
            for (let i = 0; i < 5; i++) {
        ...
        if (w.biome === 'Abyss') {
            // Floating platforms. Worst case (bounds 120x120, half-extent up
            // to s/2=3 by (s*1.3)/2=3.9): half-range 84 + 3.9 = 87.9 vs 120 --
            // 32-unit margin. Never actually capable of exceeding bounds.
            for (let i = 0; i < 10; i++) {
            ...
            // Void energy beams. Worst case: half-range 72 + radius 0.1 =
            // 72.1 vs 120 -- a 48-unit margin. Never at risk.
            for (let i = 0; i < 5; i++) {
                const x = (rng() - 0.5) * w.bounds.x * 1.2;
                const z = (rng() - 0.5) * w.bounds.z * 1.2;
        ...
        if (w.biome === 'Desert') {
            // Oasis patches with palm trees. Worst case (bounds 150x150):
            // half-range 90 + palm offset r*0.8(4) + palm canopy radius(~1.05)
            // = ~95 vs 150 -- a 55-unit margin. Never at risk.
            for (let i = 0; i < 3; i++) {
                const x = (rng() - 0.5) * w.bounds.x * 1.2;
                const z = (rng() - 0.5) * w.bounds.z * 1.2;
                if (Math.sqrt(x * x + z * z) < 20) continue;
        ...
        if (w.biome === 'Terra') {
            // Ponds. Worst case (bounds 150x150, r up to 6): half-range 90 +
            // 6 = 96 vs 150 -- a 54-unit margin. Never at risk.
            for (let i = 0; i < 4; i++) {
                const x = (rng() - 0.5) * w.bounds.x * 1.2;
                const z = (rng() - 0.5) * w.bounds.z * 1.2;
                if (Math.sqrt(x * x + z * z) < 20) continue;
