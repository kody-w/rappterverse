...
const RappterVM = {
    ...
    // ── Compile Agent Behaviors ──
    // Two-tier behavior: sloshed routines (server-generated between frames)
    // + local mood/reflex programs. Even if no new frame ever arrives,
    // agents keep living from the last data slosh's routines.
    //
    // PATTERN: "Reflex vs Thought"
    //   Frame-based thoughts: LLM generates actions once per frame (~5 min)
    //   Mid-frame reflexes:   Data slosh emits Lisp routines that the VM
    //                         executes 20x/sec — agents patrol, socialize,
    //                         react to proximity, follow routines. The world
    //                         stays alive between frames with zero LLM calls.
    //   Data slosh output:    game_state.worlds[wid].routines[] — array of
    //                         {agentId, program: "(lisp ...)"} compiled from
    //                         recent actions, chat pairs, relationships, goals.
    //
    _compileAgentBehaviors() {
        ...
        // Load sloshed routines from game_state (generated by data slosh phase)
        var sloshedRoutines = {};
        try {
            ...
            for (var r = 0; r < routines.length; r++) {
                ...
                if (routine.agentId && routine.program) {
                    sloshedRoutines[routine.agentId] = routine.program;
                }
            }
        } catch(e) { /* no routines yet */ }

        // Load recent chat pairs for social attraction
        var chatPairs = {};
        try {
            var msgs = GameState.data.chat || [];
            for (var m = Math.max(0, msgs.length - 20); m < msgs.length; m++) {
                var msg = msgs[m];
                var authorId = msg.author ? (msg.author.id || '') : '';
                if (authorId && msg.world === GameState.currentWorld) {
                    if (!chatPairs[authorId]) chatPairs[authorId] = [];
                    // Find other speakers in same world
                    for (var n = Math.max(0, msgs.length - 20); n < msgs.length; n++) {
                        if (n === m) continue;
                        var other = msgs[n];
                        var otherId = other.author ? (other.author.id || '') : '';
                        if (otherId && otherId !== authorId && other.world === msg.world) {
                            if (chatPairs[authorId].indexOf(otherId) === -1) chatPairs[authorId].push(otherId);
                        }
                    }
                }
            }
        ...
        agents.forEach(function(agent) {
            ...
            // ── Tier 1: Sloshed routines (server-generated, high priority) ──
            if (sloshedRoutines[id]) {
                try {
                    var parsed = self.parse(sloshedRoutines[id]);
                    for (var p = 0; p < parsed.length; p++) program.push(parsed[p]);
                } catch(e) { /* bad routine, skip */ }
            }

            // ── Tier 2: Social attraction (approach recent chat partners) ──
            var partners = chatPairs[id] || [];
            if (partners.length > 0) {
                var partner = partners[Math.floor(Math.random() * partners.length)];
                program.push(self.parse('(if (> (agent-distance "' + id + '" "' + partner + '") 4) (move-toward "' + id + '" (get (agent-pos "' + partner + '") "x") (get (agent-pos "' + partner + '") "z") 0.015) (face-toward "' + id + '" (get (agent-pos "' + partner + '") "x") (get (agent-pos "' + partner + '") "z")))')[0]);
            }

            // ── Tier 3: Mood-driven base behavior (fallback) ──
            ...
            // ── Tier 4: Periodic social approach ──
