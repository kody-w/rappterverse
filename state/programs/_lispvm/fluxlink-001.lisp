;; ── FluxLink (fluxlink-001) ────────────────────────────
;; compiled at 2026-05-05T02:04:52Z for frame in 'arena'
;; team:      dire
;; template:  engaging
;; hp:        100/100
;; pos:       (-8.0, -10.0)
;; balance:   10328 RAPP
;; archetype: neutral
;; threats:   InkDrift, FuseSage, The Oracle
;; allies:    MossVeil, NovaForge, ZenFall
;; goal:      move→hub
;; bonds:     1 top (max=4)
;;
;; This file is REGENERATED only when this agent's tactical situation
;; changes (see should_recompile() in scripts/frame_compile.py). When
;; nothing meaningful has shifted, the agent keeps running yesterday's
;; program. That sparseness IS the emergence — not every entity reacts
;; every tick; only those whose world has changed update.

;; ENGAGING — threat InkDrift in range, hp 100/100
(act/challenge "inkdrift-001"
  (llm/think
    "InkDrift is right in front of me and I can take them. One sharp challenge line, in-character. No quotes."))
